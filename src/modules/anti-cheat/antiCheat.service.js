...
const candidateAttempt = async (attemptId, userId) => {
  const attempt = await ExamAttempt.findOne({ _id: attemptId, candidate: userId, status: "IN_PROGRESS" });
...
export const logEvent = async (req, attemptId, input) => {
  const attempt = await candidateAttempt(attemptId, req.user._id);
  ...
  const log = await AntiCheatLog.create({ ...input, attempt: attempt._id, exam: exam._id, candidate: req.user._id, points, severity: severityForPoints(points), systemAction: decision.action, ipAddress: req.ip });
  ...
  if (decision.action === "AUTO_SUBMIT") {
    ...
    await AntiCheatLog.create({ attempt: attempt._id, exam: exam._id, candidate: req.user._id, eventType: "AUTO_SUBMIT_TRIGGERED", severity: "CRITICAL", points: 0, description: decision.reason, systemAction: "AUTO_SUBMIT", ipAddress: req.ip });
    if (exam.createdBy?.email) await sendAutoSubmitAlertEmail(exam.createdBy, req.user, exam, decision.reason);
...
export const uploadEvidence = async (req, attemptId, eventType) => {
  ...
  const log = await AntiCheatLog.create({ attempt: attempt._id, exam: attempt.exam, candidate: req.user._id, eventType, severity: "LOW", points: 0, description: "Monitoring evidence captured.", systemAction: "LOG_ONLY", ipAddress: req.ip, evidence: { publicId: asset.public_id, resourceType: asset.resource_type } });
  return log;
};
export const attemptLogs = async (req, attemptId, query) => {
  const attempt = await ExamAttempt.findById(attemptId);
  if (!attempt) throw new ApiError(404, "Attempt not found.");
  const permitted = req.user.role === ROLES.CANDIDATE ? String(attempt.candidate) === String(req.user._id) : await canMonitorExam(req.user, attempt.exam);
...
export const examReports = async (req, examId, query) => {
  ...
  const { page, limit, skip, sort } = paginationParams(query);
  const [data, total] = await Promise.all([AntiCheatLog.find({ exam: examId }).populate("candidate", "fullName email").sort(sort).skip(skip).limit(limit), AntiCheatLog.countDocuments({ exam: examId })]);
  return { data, meta: paginationMeta(page, limit, total) };
};
